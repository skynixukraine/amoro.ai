...
import { StandardFormValuesType } from '@/common/types';
...
export default function ExportTemplate({
  ...
  const footerData = (templateData.formData as StandardFormValuesType).footers;
  ...
  const pdfFileName = `standard_${timeStamp}.pdf`;
  const htmlFileName = `standard_${timeStamp}.zip`;
  const htmlRegularName = `standard_${timeStamp}.html`;
  ...
  const saveExported = (type: string, template: string) => {
    ...
    console.log('Saving export to database', templateData);
    axios
      .post('/api/exports', {
        user_id: userId,
        exportType: type,
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (htmlContent) {
        const data = await replaceImgSrcWithBlobUrl(htmlContent);
        setContent(data);
      }
    };
  ...
  const DownloadPdfButton = () => {
    return checkExport && content ? (
      <PDFDownloadLink
        ...
        onClick={() => saveExported('pdf', 'standard')}
