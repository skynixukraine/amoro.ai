...
export default function OptimizerLayout({
  ...
  const updateBannerImage = useCallback(
    (imageUrl: string | null) => {
      ...
      updatePageContext({
        formData: { ...formData, bannerImage: imageUrl },
      });
    },
    [updatePageContext, templateData]
  );
  ...
  async function onNextButonClick() {
    ...
    const previewEmail = buildEmailTemplateMarkup({
      ...
      imageWidth,
      theme:
        previewMode === 'desktop' ? templateData.layout.variation : undefined,
      unsubscribeLink:
        ...
        <Box key={index} width={isMobile ? '100%' : 'auto'}>
          <div
            className={`theme-select ${
              (templateData.layout.variation === item.value ||
                (!templateData.layout.variation && item.value === 'origin')) &&
              'theme-selected'
            }`}
            style={{
