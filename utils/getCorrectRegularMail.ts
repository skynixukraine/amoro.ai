...
const Paragraph = ({ text, isWrap }: { text: string; isWrap?: boolean }) => {
  ...
  <td valign="top" ${
    isWrap ? "width='50%'" : ''
  } style="font-family: Arial, Helvetica, sans-serif; font-size: 14px; line-height: 1.5; font-weight: normal; color: #333333; text-align: left;">
...
const Image = ({
  ...
  imageData: OptimizerFormFieldType;
  index: number;
  isWrap?: boolean;
}) => {
            ...
            <table cellpadding="0" cellspacing="0" border="0" align="center">
              <tr>
                <td style="position: relative;">
                  <img src="${file}" width="450" style="width:100%; max-width:450px; display: block; height: auto"  alt="" border="0"/>
                </td>
               </tr>
            </table>
            </a>
...
export default async function buildCorrectVeevaMail({
  ...
  if (footerData[0].image) {
    ...
    if (footerData[0].image instanceof Blob) {
      footerExtension = footerData[0].image.name.split('.').pop() || '';
      imagesFolder.file(
        `footer-image-0.${footerExtension}`,
        footerData[0].image
      );
    } else {
  ...
  const contentData = emailData.map((item, idx) => {
    if (
      item.type === 'paragraph' &&
      item.key !== 'referenceText' &&
      item.value
  ...
  const contentDataMobile = emailData.map((item, idx) => {
    if (
      item.type === 'paragraph' &&
      item.key !== 'referenceText' &&
      item.value
  ...
  <td valign="top">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td valign="top">
          <table width="100%" border="0" cellspacing="0" cellpadding="0" ${
            !variation || variation !== 'origin' ? "class='desktop-only'" : ''
          }>
            ${
              bannerImage
                ? `<tr>
                  <td valign="top" align="center" style="position: relative;">
                    <img src="${bannerImage}" width="700" alt="" style="width:100%; max-width: 700px;height: auto; display: block;" border="0"/>
                  </td>
                </tr>`
                : ''
            }
            ...
            ${contentData.join(`
              ...
              `)}
            <tr>
              <td valign="top" style="font-family: Arial, Helvetica, sans-serif; font-size: 14px; line-height: 18px; font-weight: normal; color: #333333; text-align: left;${
                ...
                ${
                  referencesAndCitations
                    ? `${referencesAndCitations.replaceAll('\n', '<br/>')}`
                    : ''
                }
              </td>
            </tr>
            ...
            <tr>
              ...
              footerData[0].text.length > 0 || footerData[0].image
                ? `<table width="100%" border="0" cellspacing="0" cellpadding="0">
                      ...
                      ${
                        footerExtension
                          ? `<img src="${footerData[0].image}" width="140" height="auto" alt="" style="display: block; margin: 0 auto;" border="0"/>`
                          : ''
                      }</th>
		                  <th width="10"></th>
          ...
          ${
            ...
            ${
              bannerImage
                ? `<tr>
                  <td valign="top" align="center">
                    <img src="${bannerImage}" alt="" width="700" style="width:100%; max-width:700px; height: auto; display: block; " border="0"/>
                  </td>
                </tr>`
                : ''
            }
...
${
  pi.length > 0
    ...
    <tr>
		${
      privacyPolicy
        ? `<td valign="center" height='30' style="font-family: Arial, Helvetica, sans-serif; font-size: 13px; line-height: 18px; font-weight: normal; text-align: center;"><a href="${privacyPolicy} " target="_blank" style="text-decoration: underline;color:#000">PrivacyPolicy</a></td>`
        : ''
    }

		${
      unsubscribeLink
        ? `<td valign="center" height='30' style="font-family: Arial, Helvetica, sans-serif; font-size: 13px; line-height: 18px; font-weight: normal; text-align: center; padding-left: 10px"><a href="${unsubscribeLink}" target="_blank" style="text-decoration: underline;color:#000">Unsubscribe</a></td>`
        : ''
    }
		</tr>
		...
		<tr><td height="20"></td></tr>
		<tr align="center">
		  <td valign="top" style="font-family: Arial, Helvetica, sans-serif; font-size: 13px; line-height: 18px; font-weight: normal; color: #333333; text-align: center;">${
        veevaApproval || ''
      }</td></tr>
	</table>
	</td></tr>
...
<![endif]-->
    ...
    `;
  //console.log(html)
  const mainContent = new Blob([html], { type: 'text/html' });
